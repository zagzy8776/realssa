    ...
    match tokio::net::lookup_host(format!("{}:80", host)).await {
        Ok(mut addrs) => {
            if let Some(addr) = addrs.next() {
                if private.iter().any(|p| addr.ip().to_string().starts_with(p)) { return false; }
            }
            true
        }
...
async fn render_page(
    ...
    if !is_safe_url(&url).await {
        return (StatusCode::FORBIDDEN, Json(serde_json::json!({"success": false, "error": "Blocked URL"}))).into_response();
    }
        ...
        Ok(r) if r.status().is_success() => {
            ...
            match r.text().await {
                ...
                Err(e) => {
                    ...
                    return Json(types::ParseResult::proxy_fallback(&url, "read_error", Default::default())).into_response();
                }
            }
        }
        Ok(r) => {
            warn!("Upstream {} → {}", url, r.status());
            return Json(types::ParseResult::proxy_fallback(&url, "upstream_error", Default::default())).into_response();
        }
        Err(e) => {
            warn!("Fetch failed {}: {}", url, e);
            return Json(types::ParseResult::proxy_fallback(&url, "fetch_error", Default::default())).into_response();
        }
    };
    let engine_base = state.engine_base.clone();
    let url_clone = url.clone();
    let result = tokio::task::spawn_blocking(move || parser::parse_page(&html, &url_clone, &engine_base))
        .await
        .unwrap_or_else(|_| types::ParseResult::proxy_fallback(&url, "parse_panic", Default::default()));
    state.render_cache.insert(cache_key.clone(), CacheEntry::new(result.clone()));
    if state.render_cache.len() > 500 {
        let mut keys: Vec<(String, Instant)> = state.render_cache.iter().map(|e| (e.key().clone(), e.value().inserted)).collect();
        keys.sort_by_key(|(_, t)| *t);
        for (k, _) in keys.iter().take(100) { state.render_cache.remove(k); }
    }
...
async fn img_handler(
    ...
    let img_url = if img_url.starts_with("//") { format!("https:{}", img_url) }
        else if !img_url.starts_with("http") { format!("https://{}", img_url) }
        else { img_url };
    ...
    match img::compress_image(&img_url, max_w, quality).await {
        Ok((webp_bytes, orig_size, compressed_size)) => {
            let saving = if orig_size > 0 { 100 - (compressed_size * 100 / orig_size) } else { 0 };
            info!("[img] {} → {}KB → {}KB WebP ({}% saved)",
                reqwest::Url::parse(&img_url).map(|u| u.host_str().unwrap_or("?").to_string()).unwrap_or_default(),
                orig_size / 1024, compressed_size / 1024, saving);
            state.img_cache.insert(cache_key.clone(), CacheEntry::new(webp_bytes.clone()));
            if state.img_cache.len() > 200 {
                let mut keys: Vec<(String, Instant)> = state.img_cache.iter().map(|e| (e.key().clone(), e.value().inserted)).collect();
                keys.sort_by_key(|(_, t)| *t);
                for (k, _) in keys.iter().take(50) { state.img_cache.remove(k); }
            }
            let mut headers = HeaderMap::new();
            headers.insert("Content-Type", HeaderValue::from_static("image/webp"));
            headers.insert("Cache-Control", HeaderValue::from_static("public, max-age=300"));
            headers.insert("X-RealSSA-Cache", HeaderValue::from_static("MISS"));
            headers.insert("X-RealSSA-Saved", HeaderValue::from_str(&format!("{}%", saving)).unwrap());
...
async fn proxy_page_handler(headers: HeaderMap, Query(params): Query<UrlQuery>) -> Response {
    let url = params.url.trim().to_string();
    let url = if url.starts_with("http") { url } else { format!("https://{}", url) };
    if !is_safe_url(&url).await { return (StatusCode::FORBIDDEN, "Blocked").into_response(); }
    ...
    match proxy::proxy_page(&url, cookie_val).await {
        Ok((html, set_cookies)) => {
            ...
            for cookie in set_cookies {
                if let Ok(v) = HeaderValue::from_str(&cookie) { headers.append("Set-Cookie", v); }
            }
            (headers, html).into_response()
        }
        Err(e) => {
            warn!("[proxy] Failed {}: {}", url, e);
            let fallback = format!(r#"<h2 style="font-family:sans-serif;padding:2rem;color:#f59e0b;background:#0b0f17">Could not load page. <a href="{}" target="_blank" style="color:#60a5fa">Open directly ↗</a></h2>"#, url);
...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(std::env::var("RUST_LOG").unwrap_or_else(|_| "realssa_engine=info,tower_http=warn".to_string()))
        .init();
    ...
    let state = Arc::new(AppState { render_cache: DashMap::new(), img_cache: DashMap::new(), engine_base });
