...
const FEEDS = {
  ...
  sports: [
    'https://www.completesports.com/feed','https://soccernet.ng/feed','https://www.espn.com/espn/rss/news','https://feeds.bbci.co.uk/sport/rss.xml','https://www.skysports.com/rss/12040','https://www.theguardian.com/sport/rss','https://news.google.com/rss/search?q=sports%20news&hl=en&gl=US&ceid=US%3Aen'
  ],
  business: [
  ...
  'south-africa': ['https://www.news24.com/news24/rss','https://www.dailymaverick.co.za/dmrss','https://www.sowetanlive.co.za/rss/?publication=sowetan-live','https://news.google.com/rss/search?q=South%20Africa%20news&hl=en&gl=ZA&ceid=ZA%3Aen'],
...
async function collect(category){ const normalized=aliases[category]||category; const urls=FEEDS[normalized]||FEEDS['world']; const feeds=await Promise.all(urls.map(readFeed)); const articles=[]; const seen=new Set(); feeds.forEach((feed,feedIndex)=>{ if(!feed) return; for(const item of feed.items.slice(0,25)){ const link=String(item.link||item.guid||'').trim(); const title=cleanText(item.title,500); if(!link||!title||seen.has(link)) continue; seen.add(link); const date=publishedDate(item); const excerpt=cleanText(item.description||title,1000); const fallback=urls[feedIndex]?new URL(urls[feedIndex]).hostname.replace(/^www\./,''):'RSS'; articles.push({id:link,title,excerpt,description:excerpt,original_excerpt:cleanText(excerpt,4000),image:getImage(item),author:cleanText(item.author,200),source_name:cleanText(item.source||feed.title||fallback,200),external_link:link,published_at:date.toISOString(),date:date.toISOString(),category:labels[normalized]||normalized,feed_category:normalized,country:countryByCategory[normalized]||'Global',content_type:'article',featured:false,is_featured:false}); } }); return articles.sort((a,b)=>new Date(b.published_at)-new Date(a.published_at)); }
