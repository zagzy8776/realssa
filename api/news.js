...
// Resilient read path: keep the public news feed alive when PostgreSQL is
// unavailable. Persistence/ingestion continues to use DATABASE_URL elsewhere.
const FEEDS = {
  ...
  'south-africa': ['http://feeds.news24.com/articles/news24/TopStories/rss','https://www.dailymaverick.co.za/dmrss','https://www.sowetanlive.co.za/rss/?publication=sowetan-live'],
  uk: ['http://feeds.bbci.co.uk/news/uk/rss.xml','https://www.theguardian.com/uk/rss','https://feeds.skynews.com/feeds/rss/home.xml'],
...
async function readFeed(url) {
  try {
    const response = await fetch(url, { headers: { 'User-Agent':'RealSSA-News/1.0 (+https://realssanews.com.ng)', Accept:'application/rss+xml, application/xml, text/xml, */*' }, signal: AbortSignal.timeout(5000) });
...
async function collect(category) {
  ...
  feeds.forEach((feed, feedIndex) => {
    ...
    for (const item of feed.items.slice(0,15)) {
      ...
      articles.push({ id:link, title, excerpt, original_excerpt:cleanText(excerpt,4000), image:getImage(item), author:cleanText(item.creator || item.author || '',200), source_name:cleanText(feed.title || new URL(urls[feedIndex]).hostname,200), external_link:link, published_at:date.toISOString(), date:date.toISOString(), category:normalized, content_type:'article', featured:false, is_featured:false });
    }
  });
...
module.exports = async function handler(req,res) {
  ...
  try {
    ...
    let articles;
    if (isForYou || category === 'breaking' || category === 'latest' || !category) {
    ...
    return res.status(200).json({ articles:articles.slice(0,limit), nextCursor:null, hasMore:false, source:'rss-fallback', category:category || (isForYou ? 'foryou' : 'latest') });
  } catch (error) {
    console.error('[Vercel News Fallback] handler failed:',error);
    return res.status(200).json({articles:[],nextCursor:null,hasMore:false,source:'rss-fallback',error:'News temporarily unavailable'});
  }
};
