 ...
 *
 * IMPORTANT: All fetch calls must use ABSOLUTE URLs (https://realssanews.com.ng/...)
 * Relative paths like '/api/foo' break inside the Android Capacitor WebView because
 * they resolve to capacitor://localhost/api/foo instead of the real server.
 */

// Absolute base URL for all API calls — works in both web browser and Capacitor native app
const BASE_URL = 'https://realssanews.com.ng';
...
export async function fetchLiveScores(timeoutMs = 2000): Promise<unknown> {
  ...
  const cacheResult = getFromCache(cacheKey);
    if (cacheResult.data) {
    if (cacheResult.isStale) revalidateInBackground(`${BASE_URL}/api/scores`, cacheKey, CACHE_CONFIG.liveScores);
    return cacheResult.data;
  }
  
  try {
    return await fetchEdgeAPI(`${BASE_URL}/api/scores`, cacheKey, CACHE_CONFIG.liveScores, timeoutMs);
  } catch {
...
export async function fetchHomepageContent(): Promise<{ headlines: unknown[]; liveScores: unknown; featured: unknown[] }> {
  const [headlines, liveScores, featured] = await Promise.all([
    ...
    fetchEdgeAPI(`${BASE_URL}/api/articles/featured`, 'featured-articles', CACHE_CONFIG.newsFeed).catch(() => []),
  ]);
