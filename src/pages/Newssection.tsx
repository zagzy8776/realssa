  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        // Apply category filter if provided (e.g. for /entertainment route)
        const filteredArticles = categoryFilter
          ? uniqueArticles.filter((a: NewsArticle) =>
              (a.category || '').toLowerCase().includes(categoryFilter.toLowerCase()) ||
              (a.title || '').toLowerCase().includes(categoryFilter.toLowerCase())
            )
          : uniqueArticles;

        setNews(filteredArticles.length > 0 ? filteredArticles : uniqueArticles);
      } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="relative overflow-hidden mb-12 rounded-2xl">
        <div className="absolute inset-0">
          <SimpleImage
            src="https://images.unsplash.com/photo-1504711432869-001077659a9a?w=800&q=75"
            alt="News Hero"
            className="w-full h-full object-cover"
            fallback="https://placehold.co/800x400/1a1a1a/ffffff?text=Latest+News"
          />
      ...
      <section id="latest" className="mb-12">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[...Array(4)].map((_, index) =>
              <div key={index} className="bg-white/dark:bg-[#211D26] border border-border rounded-lg p-6 shadow-lg animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {news.map((article, index) => {
              ...
              return (
                <article key={article.id || index} className="bg-card text-foreground rounded-lg p-6 shadow-lg hover:shadow-xl transition-shadow duration-300 border border-border">
                  ...
                  <h3 className="text-foreground text-xl font-bold mb-2 line-clamp-2">{article.title}</h3>
