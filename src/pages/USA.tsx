...
const USA = () => {
  ...
  const [searchResults, setSearchResults] = useState<USANewsItem[] | null>(null);
  ...
  const totalPages = Math.ceil(usaNews.length / ARTICLES_PER_PAGE);
  const paginatedNews = usaNews.slice(
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title="USA News for Africa — American Headlines | RealSSA News"
        description="Get the latest US and American news coverage relevant to Africa. Washington DC, New York and global US politics on RealSSA News."
        keywords="USA news Africa, American news, US breaking news, RealSSA USA, US politics Africa, Washington news, New York news"
        url="/usa"
        section="USA"
      />
      <Header />
      ...
      <section className="relative overflow-hidden mb-12">
        <div className="absolute inset-0">
          <SimpleImage
            src="https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=800&q=75"
            alt="USA News"
            className="w-full h-full object-cover"
            fallback="https://placehold.co/800x400/000000/ffffff?text=USA+News"
          />
        ...
        {!loading && usaNews.length > 0 && (
          <>
            ...
            <p className="text-center text-sm text-muted-foreground mb-4 mt-4">
              Page {currentPage} of {totalPages} · {usaNews.length} articles
            </p>
          </>
        )}
      </section>
