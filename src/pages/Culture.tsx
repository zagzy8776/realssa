...
const Culture = () => {
  ...
  useEffect(() => {
    const fetchCultureArticles = async () => {
      setLoading(true);
      try {
        // Use dedicated culture RSS endpoint for rich Variety, Rolling Stone, Essence, OkayAfrica, etc.
        const response = await fetch(apiUrl('/api/news/culture'));
        if (response.ok) {
          const articles = await response.json();
          setCultureArticles(articles);
        } else {
          console.warn("Failed to fetch culture articles from API");
        }
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title="African Culture, Nollywood & Entertainment News | RealSSA News"
        description="Explore African culture, Nollywood movies, Afrobeats music, fashion and traditions. RealSSA News covers the best of African entertainment and cultural stories."
        keywords="Nollywood news, Afrobeats, African culture, African fashion, African music, RealSSA culture, African entertainment, Nigerian music, Afrobeats latest, African heritage"
        url="/culture"
        section="Culture"
      />
      <Header />
