        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              onClick={() => {
                const url = encodeURIComponent(article.externalLink || window.location.href);
                const text = encodeURIComponent(`Hot take on: ${article.title}`);
                window.open(`https://buffer.com/add?url=${url}&text=${text}`, '_blank', 'noopener,noreferrer');
              }}
              className="flex items-center gap-2 bg-[#2C4BFF]/10 text-[#2C4BFF] hover:bg-[#2C4BFF]/20 border-[#2C4BFF]/20"
            >
